...
namespace RPC //role play colours, switched from if to switch case
{
	enum color
	{
		color_green_main_spawn = 0xFF00ff9d,
		color_purple_main_spawn = 0xFFFF009D,
		color_orange_main_spawn = 0xFFFFCD61,
		color_undead_main_spawn = 0xFF737373,

		color_deposit_coal_s = 0xFFdc8961,
		color_deposit_coal_m = 0xFFc4724e,
		color_deposit_coal_l = 0xFF984821,
		color_deposit_iron_s = 0xFF977361,
		color_deposit_iron_m = 0xFF7b5f51,
		color_deposit_iron_l = 0xFF665045,
		color_deposit_mythril_s = 0XFF7085c9,
		color_deposit_mythril_m = 0xff6174b3,
		color_deposit_mythril_l = 0xff4d62a6,
		color_deposit_adamantite_l = 0xff42b58b,
		color_deposit_random = 0xffc889c8,
		color_deposit_random_rare = 0xffC864C8,

		color_chest_iron = 0xff8e8086,
		color_chest_steel = 0xffb8a9b0,
		color_chest_gilded = 0xffc88925,
		color_chest_mythril = 0xffc889a6,
		color_chest_adamant = 0xff50d09b,
		color_chest_random = 0xff89ff89,

        color_tree_redwood = 0xff356722,
        color_cactus = 0xff937f0e,

        color_zombie_portal = 0xffff8080,
        color_skeleton = 0xff808080,
        color_zombie = 0xff9f9797,
        color_zombie_knight = 0xff806565,

 
		color_dungeon_entrance = 0xff4f2558,
		color_dungeon_exit = 0xff673f70,
		color_dungeon_chest = 0xffc41112,
		color_dungeon_chest_noom = 0xff969595,
		color_dungeon_door_holy = 0xffD7B66C,
		color_dungeon_door = 0xffd7d7d7,
		color_necromancer_minion = 0xff2c0e3a,
		color_mini_portal = 0xff804040
	};
}
...
class PNGLoader
{
	...
	void handlePixel(const SColor &in pixel, int offset)
	{
		...
		else
		{
			...
			case map_colors::princess:             autotile(offset); spawnBlob(map, "princess",    offset, 4); break;
			case map_colors::necromancer:          autotile(offset); spawnBlob(map, "noom", offset, 5); break;
			...
			default:
				handleRoleplayTile( map, offset, pixel );
			};
		}
	}
	
	void handleRoleplayTile(CMap@ map, int offset, SColor pixel)
	{

		switch (pixel.color)
		{
			case RPC::color_green_main_spawn:  AddMarker( map, offset, "green main spawn" ); break;
			case RPC::color_purple_main_spawn: AddMarker( map, offset, "purple main spawn" ); break;
			case RPC::color_orange_main_spawn: AddMarker( map, offset, "orange main spawn" ); break;
			case RPC::color_undead_main_spawn: AddMarker( map, offset, "undead main spawn" ); break;

			case RPC::color_deposit_coal_s: spawnBlob( map, "coal_deposit_s", offset, -1, true); map.SetTile(offset, CMap::tile_ground_back ); autotile(offset); break;
			case RPC::color_deposit_coal_m: spawnBlob( map, "coal_deposit_m", offset, -1, true); map.SetTile(offset, CMap::tile_ground_back ); autotile(offset); break;
			case RPC::color_deposit_coal_l: spawnBlob( map, "coal_deposit_l", offset, -1, true); map.SetTile(offset, CMap::tile_ground_back ); autotile(offset); break;

			case RPC::color_deposit_iron_s: spawnBlob( map, "iron_deposit_s", offset, -1, true); map.SetTile(offset, CMap::tile_ground_back ); autotile(offset); break;
			case RPC::color_deposit_iron_m: spawnBlob( map, "iron_deposit_m", offset, -1, true); map.SetTile(offset, CMap::tile_ground_back ); autotile(offset); break;
			case RPC::color_deposit_iron_l: spawnBlob( map, "iron_deposit_l", offset, -1, true); map.SetTile(offset, CMap::tile_ground_back ); autotile(offset); break;
			
			case RPC::color_deposit_mythril_s: spawnBlob( map, "mythril_deposit_s", offset, -1, true); map.SetTile(offset, CMap::tile_ground_back ); autotile(offset); break;
			case RPC::color_deposit_mythril_m: spawnBlob( map, "mythril_deposit_m", offset, -1, true); map.SetTile(offset, CMap::tile_ground_back ); autotile(offset); break;
			case RPC::color_deposit_mythril_l: spawnBlob( map, "mythril_deposit_m", offset, -1, true); map.SetTile(offset, CMap::tile_ground_back ); autotile(offset); break;

			case RPC::color_deposit_adamantite_l: spawnBlob( map, "adamantite_deposit_l", offset, -1, true); map.SetTile(offset, CMap::tile_ground_back ); autotile(offset); break;

			case RPC::color_chest_iron: spawnBlob( map, "chest_iron", offset, 5 ); autotile(offset); break;
			case RPC::color_chest_steel: spawnBlob( map, "chest_steel", offset, 5 ); autotile(offset); break;
			case RPC::color_chest_gilded: spawnBlob( map, "chest_gilded", offset, 5 ); autotile(offset); break;
			case RPC::color_chest_mythril: spawnBlob( map, "chest_mythril", offset, 5 ); autotile(offset); break;
			case RPC::color_chest_adamant: spawnBlob( map, "chest_adamant", offset, 5 ); autotile(offset); break;

			case RPC::color_tree_redwood: autotile(offset); offsets[tree_redwood_offset].push_back(offset); break;
			case RPC::color_cactus: spawnBlob( map, "cactus", offset, -1, true);autotile(offset); break;

			case RPC::color_zombie_portal: map.SetTile(offset, CMap::tile_castle_back_moss ); spawnBlob( map, "zombie_portal", offset, 5); autotile(offset); break;
			case RPC::color_skeleton: map.SetTile(offset, CMap::tile_castle_back ); spawnBlob( map, "skeleton", offset, 5); autotile(offset); break;
			case RPC::color_zombie: map.SetTile(offset, CMap::tile_castle_back ); spawnBlob( map, "zombie", offset, 5); autotile(offset); break;
			case RPC::color_zombie_knight: map.SetTile(offset, CMap::tile_castle_back ); spawnBlob( map, "zombie_knight", offset, 5); autotile(offset); break;
			case RPC::color_dungeon_entrance: spawnBlob( map, "dungeon_entrance", offset, -1); autotile(offset); break;
			case RPC::color_dungeon_exit: spawnBlob( map, "dungeon_exit", offset, -1); autotile(offset); break;
			case RPC::color_dungeon_door_holy: spawnBlob( map, "dungeon_door", offset, 4, true ); autotile(offset); break;
			case RPC::color_dungeon_door: spawnBlob( map, "dungeon_door", offset, 5, true ); autotile(offset); break;
			case RPC::color_dungeon_chest: spawnBlob( map, "chest_dungeon", offset, -1); autotile(offset); break;
			case RPC::color_dungeon_chest_noom: spawnBlob( map, "chest_noom", offset, -1); autotile(offset); break;
			case RPC::color_necromancer_minion: AddMarker( map, offset, "necromancer minion spawner" ); autotile(offset); break;
			case RPC::color_mini_portal: map.SetTile(offset, CMap::tile_castle_back_moss ); spawnBlob( map, "mini_portal", offset, 5); autotile(offset); break;

			case RPC::color_deposit_random:
			{
				// prev: 5.0% adamant, 10.0% mythril, 10.0% crystal, 35.0% coal, 40.0% iron
				// curr: 6.0% adamant, 12.0% mythril, 12.0% crystal, 30.0% coal, 40.0% iron
				int oreRand = map_random.NextRanged(1000);
				string ore = oreRand < 60 ? "adamantite" : 
							 oreRand < 180 ? "mythril" : 
							 oreRand < 300 ? "crystal" : 
							 oreRand < 600 ? "coal" : 
											 "iron";
				
				if (map_random.NextRanged(1000) < 600)
				{
			...
			case RPC::color_deposit_random_rare:
			{
			    // prev: 10% adamant, 20% mythril, 20% crystal, 30% coal, 20% iron
				// curr: 14% adamant, 28% mythril, 28% crystal, 20% coal, 10% iron
				int oreRand = map_random.NextRanged(1000);
				string ore = oreRand < 140 ? "adamantite" : 
							 oreRand < 420 ? "mythril" : 
							 oreRand < 700 ? "crystal" : 
							 oreRand < 900 ? "coal" : 
											 "iron";
			...
			case RPC::color_chest_random:
			{
				...
				*/
				...
				/*
				string chest = chestRand < 10 ? "adamant" : 
							   chestRand < 25 ? "mythril" : 
							   chestRand < 45 ? "crystal" :
							   chestRand < 60 ? "gilded" : 
							   chestRand < 80 ? "steel" :
												"iron";*/
												
				string chest = chestRand < 10 ? "adamant" : 
							   chestRand < 20 ? "mythril" : 
							   chestRand < 30 ? "gilded" : 
							   chestRand < 50 ? "crystal" :
							   chestRand < 70 ? "steel" : 
												"iron";
												
				if (map_random.NextRanged(100) < 35)
				{
			...
			default: HandleCustomTile( map, offset, pixel ); break;
		}
	}
...
void SaveMap(CMap@ map, const string &in fileName)
{
	...
	//Dont need whats above
	//We need to run the 'hook' below
	//AutoMapSaver.saveMap(map);
}

void getInfoFromBlob(CBlob@ this, SColor &out color, Vec2f &out offset)
{
	const string name = this.getName();
	const int nHash   = this.getName().getHash();
