...
bool onServerProcessChat( CRules@ this, const string& in text_in, string& out text_out, CPlayer@ player )
{
	...
	const bool aphelion = name == "Aphelion";
	const bool superadmin = getSecurity().getPlayerSeclev(player).getName() == "Super Admin" || getSecurity().getPlayerSeclev(player).getName() == "Hidden Super Admin";
	...
	if (aphelion)
	{
		...
		else if(args[0] == "/nocap")
		{
			string context = args[1];
			
			getRules().set_bool("nocap", context != "on" ? true : false);
			getRules().Sync("nocap", true);
		}
	}
	...
	if (text_in == "/togglehead")
	{
		...
		string tag = head_tag + " " + name;
		
	    bool head = !rules.hasTag(tag);
		if  (head)
		{
			rules.Tag(tag);
			rules.Sync(tag, true);
	...
	{
		CRules@ rules = getRules();
		string tag = skin_tag + " " + name;
		
	    bool skin = !rules.hasTag(tag);
		if  (skin)
		{
			rules.Tag(tag);
			rules.Sync(tag, true);
	...
	if (args[0] == "!countof" && aphelion)
    {
        // print all blobs
        CBlob@[] all;
        getBlobsByName(args[1], @all);
		printf("COUNT OF " + args[1] + ": " + all.length);
    }
