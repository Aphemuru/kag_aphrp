...
{
	//sort players
	CPlayer@[] humanplayers;
	CPlayer@[] dwarfplayers;
	CPlayer@[] elfplayers;
	CPlayer@[] orcplayers;
	CPlayer@[] angelplayers;
	CPlayer@[] undeadplayers;
	CPlayer@[] spectators;
	for (u32 i = 0; i < getPlayersCount(); i++)
	{
		...
		{
			for (u32 j = 0; j < humanplayers.length; j++)
			{
				if (getKDR(humanplayers[j]) < kdr)
				{
					humanplayers.insert(j, p);
					inserted = true;
					break;
				}
			}
		...
		{
			for (u32 j = 0; j < dwarfplayers.length; j++)
			{
				if (getKDR(dwarfplayers[j]) < kdr)
				{
					dwarfplayers.insert(j, p);
					inserted = true;
					break;
				}
			}
			...
			if (!inserted)
				dwarfplayers.push_back(p);

		}
		else if (teamNum == 2)
		{
			for (u32 j = 0; j < elfplayers.length; j++)
			{
				if (getKDR(elfplayers[j]) < kdr)
				{
					elfplayers.insert(j, p);
					inserted = true;
					break;
				}
			}
			...
			if (!inserted)
				elfplayers.push_back(p);
		}
		else if (teamNum == 3)
		{
			for (u32 j = 0; j < orcplayers.length; j++)
			{
				if (getKDR(orcplayers[j]) < kdr)
				{
					orcplayers.insert(j, p);
					inserted = true;
					break;
				}
			}
			...
			if (!inserted)
				orcplayers.push_back(p);
		}
		else if (teamNum == 4)
		{
			for (u32 j = 0; j < angelplayers.length; j++)
			{
				if (getKDR(angelplayers[j]) < kdr)
				{
					angelplayers.insert(j, p);
					inserted = true;
					break;
				}
			}
			...
			if (!inserted)
				angelplayers.push_back(p);
		}
		else if (teamNum == 5)
		{
			for (u32 j = 0; j < undeadplayers.length; j++)
			{
				if (getKDR(undeadplayers[j]) < kdr)
				{
					undeadplayers.insert(j, p);
					inserted = true;
					break;
				}
			}
	...
	{
	case 1:
		topleft.y = drawScoreboard(localPlayer, dwarfplayers, topleft, this.getTeam(1), Vec2f_zero);
		topleft.y += 52;
		topleft.y = drawScoreboard(localPlayer, humanplayers, topleft, this.getTeam(0), Vec2f_zero);
		topleft.y += 52;
		topleft.y = drawScoreboard(localPlayer, elfplayers, topleft, this.getTeam(2), Vec2f_zero);
		topleft.y += 52;
		topleft.y = drawScoreboard(localPlayer, orcplayers, topleft, this.getTeam(3), Vec2f_zero);
		topleft.y += 52;
		topleft.y = drawScoreboard(localPlayer, angelplayers, topleft, this.getTeam(4), Vec2f_zero);
		topleft.y += 52;
		topleft.y = drawScoreboard(localPlayer, undeadplayers, topleft, this.getTeam(5), Vec2f_zero);
		break;
