	...
	if(cmd == this.getCommandID(cmd_exit_dungeon))
	{
		...
		if (caller !is null)
		{
			...
			if (entrances.length > 0)
			{
				...
				// immunity for 3s
				caller.Tag("invincible");
				caller.Untag("invincibility done");
				caller.set_u32("spawn immunity time", getGameTime());
				caller.Sync("invincible", true);
				caller.Sync("invincibility done", true);
				caller.Sync("spawn immunity time", true);
