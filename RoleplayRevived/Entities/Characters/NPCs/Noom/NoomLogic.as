...
void onInit( CBlob@ this )
{
	...
	random.Reset(this.getNetworkID() * 139 + getGameTime() * 7);
}
	...
	{	
		...
		if (distance < 256.0f)
		{
			...
			if (hpPercentage < 25)
			{
				...
				// set the regeneration rate to 4.0HP/3s
				blob.set_f32("regen rate", 2.00f);
			}
			else if(hpPercentage < 50)
			{
			    phase = AttackPhase::phase_three;
				
				// set the regeneration rate to 3.0HP/3s
				blob.set_f32("regen rate", 1.50f);
			}
			else if(hpPercentage < 75)
			{
			    phase = AttackPhase::phase_two;
				
				// set the regeneration rate to 1.5HP/3s
				blob.set_f32("regen rate", 0.75f);
			}
			else
			{
			    phase = AttackPhase::phase_one;
				
				// set the regeneration rate to 1.0HP/3s
				blob.set_f32("regen rate", 0.50f);
			}
...
void DoTeleport( CBlob@ blob, u8 phase )
{
	...
	if   (map.getMarkers("necromancer teleport", teleports)) 
	{
		...
		//bool teleportRandomly = XORRandom(512) < (phase > AttackPhase::phase_two ? 96 : 128);
		bool teleportRandomly = random.NextRanged(512) < (phase > AttackPhase::phase_two ? 96 : 128);
		if  (teleportRandomly)
		{
		    //nextTeleport = teleports[XORRandom(teleports.length)];
			nextTeleport = teleports[random.NextRanged(teleports.length)];
		}
		else
		{
