...
class CutState : KnightState
{
	...
	bool TickState(CBlob@ this, KnightInfo@ knight, RunnerMoveVars@ moveVars)
	{
		...
		else if (delta > DELTA_BEGIN_ATTACK && delta < DELTA_END_ATTACK)
		{
			...
			DoAttack(this, 1.0f, attackAngle, attackarc, XORRandom(512) < 128 ? Hitters::mace_power : Hitters::mace, delta, knight);
		}
		else if (delta >= 9)
		{
...
class SlashState : KnightState
{
	...
	bool TickState(CBlob@ this, KnightInfo@ knight, RunnerMoveVars@ moveVars)
	{
		...
		else if (delta > DELTA_BEGIN_ATTACK && delta < 10)
		{
			...
			//  Mace power attack
			if (power_attack)
				DoAttack(this, 5.40f, -(vec.Angle()), 120.0f, Hitters::mace_power, delta, knight);
			else
				DoAttack(this, 2.25f, -(vec.Angle()), 120.0f, XORRandom(512) < 128 ? Hitters::mace_power : Hitters::mace, delta, knight);
		}
