...
{
    {
    	if(isClassTypeKnight(this))
		{
			...
			{   // Mace
				Equipment e( false, "mace", "$mace$", "maceman", SLOT_WEAPON, "Mace\nStab: 0.9375 Slash: 2.8125 Power: 6.75 Speed: Slow\nPower attack ignores 15 Defence rating (1 in 4 chance for normal attacks)", true );
		        AddRequirement( e.reqs, "blob", "mace", "Mace", 1 );
				equipment.push_back( e );
			}
			{   // War Axe
				Equipment e( false, "war_axe", "$war_axe$", "axeman", SLOT_WEAPON, "War Axe\nStab: 0.625 Slash: 3.125 Power: 5.875 Speed: Fast\nPower attack hits through shields (1 in 4 chance for normal attacks)", true );
	...
	
	{
		if(isClassTypeMarksman(this))
		{
			...
			{   // Crystal bow
				Equipment e( false, "crystal_bow", "$crystal_bow$", "crystalbowman", SLOT_WEAPON, "Crystal bow\nA powerful longbow forged from crystal alloy", true );
				AddRequirement( e.reqs, "blob", "crystal_bow", "Crystal bow", 1 );
				equipment.push_back( e );
			}
			{   // Musket
	...
	
	{
		if(isClassTypeMage(this))
		{
			...
			/*
			{   // Aura of the Faerie
				Equipment e( true, "aura_teleportation", "$aura_teleportation$", "mage", SLOT_ARMOUR, "Aura of the Faerie\nDefence rating of 40\nSummon a faerie\nCooldown of 30 seconds", true );
				AddRequirement( e.reqs, "blob", "aura_teleportation", "Aura of the Faerie", 1 );
				equipment.push_back( e );
			}
			{   // Aura of the Elemental
				Equipment e( true, "aura_teleportation", "$aura_teleportation$", "mage", SLOT_ARMOUR, "Aura of the Elements\nDefence rating of 40\nSummon an elemental\nCooldown of 30 seconds", true );
				AddRequirement( e.reqs, "blob", "aura_teleportation", "Aura of the Elements", 1 );
				equipment.push_back( e );
			}
			*/
			{   // Aura of Flight
			...
			{   // Mage's staff
				Equipment e( true, "default_staff", "$staff$", "mage", SLOT_WEAPON, "Mage's staff", true );
				equipment.push_back( e );
			}
			/*
			{   // Wizard's staff
			...
			{   // Druid's staff
				Equipment e( true, "default_staff", "$staff$", "mage", SLOT_WEAPON, "Druid's staff", true );
				equipment.push_back( e );
			}
			{   // Shaman's staff
				Equipment e( true, "default_staff", "$staff$", "mage", SLOT_WEAPON, "Shaman's staff", true );
				equipment.push_back( e );
			}
			*/
			/*
...
float getDamageModifier( CBlob@ this, u8 hitter )
{
	...
	if    (armour == "iron_chestplate" || armour == "iron_chainmail" || armour == "aura_teleportation")
	    ...
	    modifier = 0.4f;
	else if (armour == "gold_chestplate" || armour == "dragon_chestplate")
	    modifier = 0.35f;
	else if (armour == "armadyl_chainmail")
	    modifier = 0.3f;
	...
	if (modifier < 1.0f)
    {
		...
		else if (hitter == Hitters::crystal_arrow)
		    modifier = Maths::Min(modifier + 0.10f, 1.0f);
		    //modifier = Maths::Min(modifier + 0.00f + (XORRandom(11) / 100.0f), 1.0f);
		else if (hitter == Hitters::crystal_arrow_attuned)
		    modifier = Maths::Min(modifier + 0.20f, 1.0f);
		    //modifier = Maths::Min(modifier + 0.10f + (XORRandom(11) / 100.0f), 1.0f);
	}
	return modifier;
}
