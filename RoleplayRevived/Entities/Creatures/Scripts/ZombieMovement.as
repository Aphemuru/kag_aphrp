...
void onTick(CMovement@ this)
{
	...
	// thanks vamist
	if(this.getCurrentScript().tickFrequency == 15)//check for nearby players
	{
		CBlob@[] list;
		if(map.getBlobsInRadius(blob.getPosition(), 20.0f, @list))//Get the nearby list
		{
			for(int a = 0; a < list.length(); a++)//loop through them
			{
				CBlob@ tempBlob = list[a];//cache
				if(tempBlob is null){continue;}
				CPlayer@ p = tempBlob.getPlayer();
				if(p !is null)//look its a real player, time to attack
				{
					//might want to check if they are an enemy or not
	...
	{
		if (getGameTime() % 30 == 0)//is it time to check if something is near by or not?
		{
