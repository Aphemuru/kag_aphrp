...
f32 getPriorityPickupScale(CBlob@ this, CBlob@ b)
{
	...
	if (material)
	{
		...
		if (name == "mat_gold" || name == "mat_mythril" || name == "mat_adamantite" || name == "mat_aetheryte")
		{
			return factor_resource_strategic;
		}
			
		if (name == "mat_coal" || name == "mat_iron" || name == "mat_flour")
		{
		...
		const bool knight = isClassTypeKnight(thisname);
		...
		const bool archer = isClassTypeMarksman(thisname);
		...
		if (name == "mat_crystalarrows" || name == "mat_roundshot")
		{
			return archer ? factor_resource_strategic : factor_resource_boring;
		}
		
		if (name == "mat_waterarrows" || name == "mat_firearrows" || name == "mat_bombarrows" ||  name == "mat_ironarrows" || name == "mat_steelarrows")
		{
			return archer ? factor_resource_useful_rare : factor_resource_boring;
		}
		
		const bool mage = isClassTypeMage(thisname);
		
		if (name == "mat_energyrunes") // was orbs
		{
			// Lower priority for regular arrows when the mage has more than 12 in the inventory
			return mage && !this.hasBlob("mat_energyrune", 12) ? factor_resource_useful : factor_resource_boring;
		}
		
		if (name == "mat_lightingrunes" || name == "mat_miasmarunes" || name == "mat_bombrunes") // was water, healing, bomb orbs
		{
			return mage ? factor_resource_useful_rare : factor_resource_boring;
		}
	}
	...
	//low priority
	if (name == "log" || b.hasTag("tree") || b.hasTag("player"))
	{
