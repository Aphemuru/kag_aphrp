...
void onInit( CBlob@ this )
{
	...
	{
		Animation@ anim = sprite.addAnimation("crystal arrow",0,false);
		anim.AddFrame(24 + XORRandom(4));
		if(arrowType == ArrowType::normal_crystal)
			sprite.SetAnimation(anim);
	}
...
void onCollision( CBlob@ this, CBlob@ blob, bool solid, Vec2f normal, Vec2f point1 )
{
    if (blob !is null && doesCollideWithBlob( this, blob ) && !this.hasTag("collided"))
    {
		...
		else
		{
			...
			bool attuned = this.hasTag("attuned");
			
			if (arrowType == ArrowType::normal_crystal)
			    hitter = attuned ? Hitters::crystal_arrow_attuned : Hitters::crystal_arrow;
			...
			if (dmg > 0.0f)
			{
			    if (arrowType == ArrowType::normal)
				    ...
				    dmg *= 1.5f;
				else if (arrowType == ArrowType::normal_crystal)
				    dmg *= attuned ? 1.5f : 1.5f;
...
{
	const u8 arrowType = this.get_u8("arrow type");
	bool crossbow = isCrossbowBolt(this);
	bool crystal_arrow = arrowType == Hitters::crystal_arrow;
	bool crystal_arrow_attuned = arrowType == Hitters::crystal_arrow_attuned;
	
	// unbomb, stick to blob
	if (this !is hitBlob && (customData == Hitters::arrow || crystal_arrow || crystal_arrow_attuned))
	{
		...
		{	
			...
			int max_ticks_to_stun = crossbow ? 12 : (crystal_arrow || crystal_arrow_attuned) ? 6 : 4;
			
			if (crystal_arrow)
			    force *= 1.1f;
			if (crystal_arrow_attuned)
			    force *= 1.2f;
			if (crossbow)
				force *= 1.3f;
